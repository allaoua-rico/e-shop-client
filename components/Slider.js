...
export default function Slider() {
  const sliderImages = [
    "/slider/hm-2-slider-1.jpg",
    "/slider/hm-2-slider-2.jpg",
  ];
    ...
    <div className="relative flex items-center group bg-[#f6f6f6] ">
      <Swiper
        ...
        onTransitionEnd={({ activeIndex }) => {
          setterArr.map((setter, index) => {
            index === activeIndex ? setter(true) : setter(false);
          });
        }}
            ...
            <SwiperSlide key={image}>
              <div key={image+' '+index} className="relative overflow-hidden h-[400px] sm:h-[500px] md:h-[550px] xl:px-32 ">
                <div className="absolute mt-[100px] sm:mt-[140px] px-[35px] z-20">
                  <div
                    className={
                      "flex flex-col gap-y-2 text-[30px] sm:text-[3.25rem] tracking-wide	font-[350] translate-y-[500px] opacity-0 " +
                      (animationArr[index] &&
                        " animate-[translateOpacity_2s_ease-in-out_forwards]")
                    }
                  >
                    <div className="-mb-4">
                      <span className="text-[#628787] font-bold">Basic</span>{" "}
                      Color
                    </div>
                  ...
                  <p
                    className={
                      "text-black w-44 mt-4 sm:text-[#878786] text-sm sm:w-[340px] translate-y-[500px] opacity-0 mb-9 ml-1" +
                      (animationArr[index] &&
                        " animate-[translateOpacity_2.5s_ease-in-out_forwards]")
                    }
                  >
